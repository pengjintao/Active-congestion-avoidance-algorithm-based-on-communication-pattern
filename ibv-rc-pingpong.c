					    ...
					    int use_event)
{
	...
	int access_flags = (int) IBV_ACCESS_LOCAL_WRITE;
	...
	if (implicit_odp) {
		ctx->mr = ibv_reg_mr(ctx->pd, NULL, 1/*SIZE_MAX*/, access_flags);
	} else {
	...
	case PINGPONG_RECV_WRID:
		...
		if (use_ts) {
			if (ts->last_comp_with_ts) {
				...
				ts->comp_recv_max_time_delta = (ts->comp_recv_max_time_delta > delta ? (ts->comp_recv_max_time_delta):delta);
				ts->comp_recv_min_time_delta = (ts->comp_recv_min_time_delta < delta ? (ts->comp_recv_min_time_delta):delta); 
                //std::min(ts->comp_recv_min_time_delta, delta);
...
int ibv_pingpong_main(int rank,int nproc,MPI_Comm IPH_COMM_WORLD)
{
    ...
    if(rank == 0)
    {
        system("ifconfig ib0  2>/dev/null |grep \"inet \"|awk '{print $2}' > rank0ip.txt");
        std::ifstream fs;
        fs.open("./rank0ip.txt",std::ios::in);
        fs>>Server_ip;
        fs.close();
        //printf("my ip= %s\n",Server_ip.c_str());
        int strsize = Server_ip.size();
        MPI_Send(&strsize,1,MPI_INT,1,0,IPH_COMM_WORLD);
        MPI_Send(Server_ip.c_str(),Server_ip.size(),MPI_CHAR,1,0,IPH_COMM_WORLD);
    }
    if(rank == 1)
    {
        MPI_Status stat;
        char * servernameT = (char *)malloc(100);
        memset(servernameT,0,100);
        int strsize = 0;
        MPI_Recv(&strsize,1,MPI_INT,0,0,IPH_COMM_WORLD,&stat);
        MPI_Recv(servernameT,strsize,MPI_CHAR,0,0,IPH_COMM_WORLD,&stat);
        printf("server ip  = %s\n",servernameT);
        servername = strdupa(servernameT);
        sleep(1);
    }
...

	{
		...
		if (use_ts && ts.comp_with_time_iters) {
			printf("Max receive completion clock cycles = %uld\n",
			       ts.comp_recv_max_time_delta);
			printf("Min receive completion clock cycles = %uld\n",
